...
type SavedPreset = Partial<Preset> & { id: string };

const MODE_CONFIG: Record<ModeType, { icon: React.ComponentType<{ className?: string }>; color: string; desc: string }> = {
...
export function QuantumFieldGenerator() {
    ...
    const loadPresets = () => {
        try {
            const saved = JSON.parse(localStorage.getItem('savedPresets') ?? '[]') as SavedPreset[];
    ...
    const handleSavePreset = () => {
        try {
            const existing = JSON.parse(localStorage.getItem('savedPresets') ?? '[]') as SavedPreset[];
            const newPreset: SavedPreset = { ...params, id: crypto.randomUUID(), name: presetName };
            localStorage.setItem('savedPresets', JSON.stringify([...existing, newPreset]));
            loadPresets();
        } catch (err) {
            ...
            <header className="mb-8 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-white/60">Quantum field generator</p>
                    </div>
                </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
                <section className="space-y-6">
                    <div className="glass-panel p-5">
                        <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p className="mt-2 text-sm text-white/60">{PresetEngine.generateDescription(params)}</p>
                            </div>
                            ...
                            {(Object.keys(MODE_CONFIG) as ModeType[]).map((mode) => {
                                ...
                                return (
                                    <button key={mode} type="button" onClick={() => updateParam('mode', mode)} className={`rounded-xl border px-4 py-3 text-left transition ${params.mode === mode ? 'border-quantum-cyan bg-quantum-cyan/10' : 'border-white/10 bg-white/5 hover:bg-white/10'}`}>
                                        ...
                                        <p className="text-xs text-white/60">{MODE_CONFIG[mode].desc}</p>
                                    </button>
                                );
                            })}
                        </div>
                        ...
                        <div className="mb-4 flex gap-2">
                            {(Object.keys(PARAM_SECTIONS) as (keyof typeof PARAM_SECTIONS)[]).map((tab) => (
                                <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`rounded-lg px-4 py-2 text-sm ${activeTab === tab ? 'bg-quantum-cyan/15 text-quantum-cyan' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>
                                    {tab}
                                </button>
                            ))}
                        </div>
                            ...
                            {PARAM_SECTIONS[activeTab].map((item) => {
                                ...
                                return (
                                    <Slider
                                        ...
                                        formatValue={'formatter' in item ? item.formatter : (('unit' in item && item.unit === 'Hz') ? formatFrequency : undefined)}
                                    />
                                );
                            })}
                        </div>
                    ...
                    <div className="glass-panel p-5">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            <span className="text-xs text-white/60">{renderResult ? `${renderResult.sampleRate} Hz • peak ${renderResult.peakAmplitude.toFixed(3)}` : 'No render yet'}</span>
                        </div>
                ...
                <aside className="space-y-6">
                    <div className="glass-panel p-4">
                        <h3 className="mb-4 text-sm font-medium text-white/60">Quick presets</h3>
                    ...
                    <div className="glass-panel p-4">
                        <h3 className="mb-4 text-sm font-medium text-white/60">Saved presets</h3>
                        <div className="max-h-[320px] space-y-2 overflow-auto">
                            {savedPresets.length === 0 ? (
                                <div className="text-sm text-white/60">No saved presets yet.</div>
                            ) : savedPresets.map((preset) => (
                                <button key={preset.id} type="button" onClick={() => applyPreset(preset)} className="w-full rounded-lg border border-white/10 bg-white/5 p-3 text-left hover:bg-white/10">
                                    <div className="font-medium">{preset.name}</div>
                                    <div className="text-xs text-white/60">{Number(preset.carrierHz ?? 0).toFixed(2)} Hz • {preset.mode}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="glass-panel p-4">
                        <h3 className="mb-4 text-sm font-medium text-white/60">Active frequencies</h3>
                        <div className="space-y-3 font-mono text-sm">
                            <div className="flex justify-between"><span className="text-white/60">Carrier</span><span className="text-quantum-cyan">{formatFrequency(params.carrierHz ?? 20.51)}</span></div>
                            <div className="flex justify-between"><span className="text-white/60">Gamma</span><span className="text-quantum-magenta">{formatFrequency(params.gammaHz ?? 90.12)}</span></div>
                            <div className="flex justify-between"><span className="text-white/60">Sweep</span><span className="text-quantum-gold">{formatFrequency(params.lowerHz ?? 10.55)} - {formatFrequency(params.upperHz ?? 33.18)}</span></div>
                            <div className="flex justify-between"><span className="text-white/60">Duration</span><span>{formatDuration(params.durationSec ?? 180)}</span></div>
                            <div className="flex justify-between"><span className="text-white/60">Harmonics</span><span>{params.harmonics ?? 4}</span></div>
                        </div>
                    </div>
                </aside>
            </div>
