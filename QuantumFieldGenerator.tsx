...
export function QuantumFieldGenerator() {
  ...
  const handleSavePreset = async () => {
    if (!session?.user) {
      setError('Sign in to save presets');
      return;
    }
    try {
      ...
      <header className="mb-8 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Quantum field generator</p>
          </div>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <section className="space-y-6">
          <div className="glass-panel p-5">
            <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-muted-foreground">{PresetEngine.generateDescription(params)}</p>
              </div>
            ...
            <div className="mb-4 flex gap-2">
              {(Object.keys(PARAM_SECTIONS) as (keyof typeof PARAM_SECTIONS)[]).map((tab) => (
                <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`rounded-lg px-4 py-2 text-sm ${activeTab === tab ? 'bg-quantum-cyan/15 text-quantum-cyan' : 'bg-white/5 text-muted-foreground hover:bg-white/10'}`}>
                  {tab}
                </button>
              ))}
            </div>
          ...
          <div className="glass-panel p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">{renderResult ? `${renderResult.sampleRate} Hz • peak ${renderResult.peakAmplitude.toFixed(3)}` : 'No render yet'}</span>
            </div>
        ...
        <aside className="space-y-6">
          <div className="glass-panel p-4">
            <h3 className="mb-4 text-sm font-medium text-muted-foreground">Quick presets</h3>
          ...
          <div className="glass-panel p-4">
            <h3 className="mb-4 text-sm font-medium text-muted-foreground">Saved presets</h3>
            <div className="max-h-[320px] space-y-2 overflow-auto">
              {savedPresets.length === 0 ? <div className="text-sm text-muted-foreground">No saved presets yet.</div> : savedPresets.map((preset) => (
                <button key={preset.id} type="button" onClick={() => applyPreset(preset)} className="w-full rounded-lg border border-white/10 bg-white/5 p-3 text-left hover:bg-white/10">
                  <div className="font-medium">{preset.name}</div>
                  <div className="text-xs text-muted-foreground">{Number(preset.carrierHz).toFixed(2)} Hz • {preset.mode}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="glass-panel p-4">
            <h3 className="mb-4 text-sm font-medium text-muted-foreground">Active frequencies</h3>
            <div className="space-y-3 font-mono text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Carrier</span><span className="text-quantum-cyan">{formatFrequency(params.carrierHz ?? 20.51)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Gamma</span><span className="text-quantum-magenta">{formatFrequency(params.gammaHz ?? 90.12)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Sweep</span><span className="text-quantum-gold">{formatFrequency(params.lowerHz ?? 10.55)} - {formatFrequency(params.upperHz ?? 33.18)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Duration</span><span>{formatDuration(params.durationSec ?? 180)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Harmonics</span><span>{params.harmonics ?? 4}</span></div>
            </div>
          </div>
        </aside>
      </div>
